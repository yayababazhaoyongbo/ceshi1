...
def detect_regression_channel(df, max_period=90, direction='下降通道', require_bottom=True, min_width=30, min_touches=3, strict_close=True):
    ...
    for p in range(actual_max, 29, -5):
        ...
        # 2. 【核心重构】：抛弃标准差，计算每个 K 线极值点到中轴线的绝对距离
        dist_high = highs - reg_line
        dist_low = reg_line - lows
        
        # 3. 将通道线强行平移，精准锚定到第 min_touches 高/低的极值点上！
        # np.sort 是从小到大排序，[-min_touches] 就是倒数第 N 大的距离
        try:
        ...
        # 4. 主力底线控制：虽然是按照影线画通道，但收盘价绝不能突破这根边界盖子
        if strict_close:
            closes_outside = np.sum((y > upper_band * 1.01) | (y < lower_band * 0.99))
            if closes_outside > 0:
                continue 
        
        # 由于我们把通道强制平移到了第 min_touches 的点上，所以触碰次数天生就满足 >= min_touches
        # 这里只需要验证通道的宽度是否达标
    ...
    if st.button("📍 一键提取【直线绝对坐标】"):
        if draw_code:
            ...
            if df is not None and len(df) >= draw_period:
                ...
                st.success(f"✅ [{draw_code} {name}] 视觉级平移通道计算完成！")
